// 🚀 Nova PDF Engine V7 — Apple Style + GPT-5 Report + Auto-height + Persona + Emotional Curve
  ...
  const axis = axes
    .map(({ a }) => {
      ...
      return `<line x1="${cx}" y1="${cy}" x2="${
        cx + rMax * Math.cos(rad)
      }" y2="${cy + rMax * Math.sin(rad)}" stroke="#CBD5E1" stroke-width="1"/>`
    })
    .join("")

  const poly = `<polygon points="${points}" fill="rgba(10,92,255,0.25)" stroke="#0A5CFF" stroke-width="2"/>`

  return `<svg width="${size}" height="${size}" xmlns="http://www.w3.org/2000/svg">${grid}${axis}${poly}</svg>`
}

function extractExpectedAnswer(q: any, lang: string): string {
  // priorité : expected_answer (si tu l'as ajoutée), sinon expected_answer_en / fr, sinon ideal_answer JSON
...
export async function POST(req: NextRequest) {
  try {
    ...
    /* 2️⃣ Emotions brutes (pour radar global) */
    ...
    /* 4️⃣ Questions metadata (incl. expected_answer) */
    ...
    /* 5️⃣ Compute Behavioral Metrics (Radar 6 axes) */
    ...
    doc
      .fillColor("#FFF")
      .fontSize(30)
      .text("Interview Report", 0, 300, { align: "center" })
      .fontSize(16)
      .fillColor("#C3D5FF")
      ...
      .fontSize(11)
      .fillColor("#94A3B8")
      .text("Nova — AI Interview Coach", { align: "center" })

    doc
      .fillColor("#64748B")
      .fontSize(10)
      .text(`Session type: ${safe(session.type_entretien || "Interview")} • Global score: ${scoreGlobal}/100`, 0, 430, {
        align: "center",
      })
    ...
    doc.fillColor("#0A5CFF").fontSize(24).text("NOVA Persona Profile", 46, 50)
    ...
    if (!report) {
      report = {
        ...
        page3: {
          ...
          day7_plan: [
            "Day 1 – Rewrite 3 answers using a simple structure (context, action, result).",
            "Day 2 – Practice speaking slower by reading a text out loud.",
            "Day 3 – Prepare a 20-second pitch and repeat it 5 times.",
            "Day 4 – Add one key metric to three of your answers.",
            "Day 5 – Practice eye contact in front of a mirror for 5 minutes.",
            "Day 6 – Record yourself on 2 questions and listen back critically.",
            "Day 7 – Run a 10-minute mock interview applying all improvements.",
          ],
        ...
        page4: {
          ...
          recommendations: [
            "Add 1–2 key metrics per major experience.",
    ...
    py = block(doc, "Behavioral Mirror", p1.behavioral_mirror, py)
    ...
    doc.fillColor("#0A5CFF").fontSize(12).text("Strengths", 46, py)
    py += 20

    const strengthsText = (p1.strengths || []).map((s: string) => "• " + s).join("\n")
    ...
    doc.fillColor("#0A5CFF").fontSize(12).text("Points of Vigilance", 46, py)
    py += 20

    const risksText = (p1.risks || []).map((r: string) => "• " + r).join("\n")
    ...
    doc.rect(0, 0, 595, 842).fill("#F5F5F7")
    doc.rect(0, 0, 595, 100).fill("#FFF")
    doc.fillColor("#0A5CFF").fontSize(24).text("Emotional Trajectory", 46, 50)

    // Generate stress and confidence curves from emotions data
    const stressCurve = (emotions || []).map((e: any) => Number(e.stress) || 0)
    const confidenceCurve = (emotions || []).map((e: any) => Number(e.confidence) || 0.5)

    const stressSVG = generateCurveSVG(stressCurve, "#EF4444")
    const confSVG = generateCurveSVG(confidenceCurve, "#0A5CFF")

    doc.fillColor("#1D1D1F").fontSize(14).text("Stress Level Throughout Interview", 46, 130)
    SVGtoPDF(doc as any, stressSVG, 46, 160, { width: 500, assumePt: true })

    doc.fillColor("#1D1D1F").fontSize(14).text("Confidence Level Throughout Interview", 46, 320)
    SVGtoPDF(doc as any, confSVG, 46, 350, { width: 500, assumePt: true })
    ...
    doc.rect(0, 0, 595, 842).fill("#F5F5F7")
    doc.rect(0, 0, 595, 100).fill("#FFF")
    doc.fillColor("#0A5CFF").fontSize(24).text("Gap Analysis — Answers vs Expectations", 46, 50)
    ...
    doc.fillColor("#1D1D1F").fontSize(13).text("Key Gaps", 46, gy)
    gy += 25

    const gapsText = (p2.gaps || []).map((g: string) => "• " + g).join("\n")
    ...
    doc.fillColor("#1D1D1F").fontSize(13).text("Nova's Advice", 46, gy)
    gy += 25

    const adviceText = (p2.advice || []).map((a: string) => "• " + a).join("\n")
    ...
    doc.fillColor("#FFFFFF").fontSize(24).text("7-Day Transformation Plan", 46, 50)

    let mhY = 110

    doc.fillColor("#0A5CFF").fontSize(13).text("Micro-habits to Install", 46, mhY)
    mhY += 25

    const microHabitsText = (p3.micro_habits || []).map((m: string) => "• " + m).join("\n")
    ...
    doc.fillColor("#0A5CFF").fontSize(13).text("Day-by-Day Plan", 46, mhY)
    mhY += 25

    const dayPlanText = (p3.day7_plan || []).map((d: string) => "• " + d).join("\n")
    ...
    doc.fillColor("#0A5CFF").fontSize(13).text("Improved Pitch (Short)", 46, mhY)
    mhY += 20
    ...
    doc.fillColor("#0A5CFF").fontSize(13).text("Improved Pitch (Long)", 46, mhY)
    mhY += 20
    ...
    doc.rect(0, 0, 595, 842).fill("#F5F5F7")
    doc.rect(0, 0, 595, 100).fill("#FFF")
    doc.fillColor("#0A5CFF").fontSize(24).text("CV & Offer Fit", 46, 50)

    const fitScore = clamp(p4.cv_offer_fit_score ?? 50)

    doc.fillColor("#1D1D1F").fontSize(13).text("Global Fit Score", 46, 130)
    ...
    doc.fillColor("#1D1D1F").fontSize(13).text("Strong Alignments", 46, alY)
    alY += 25

    const alignmentText = (p4.alignment_points || []).map((a: string) => "• " + a).join("\n")
    ...
    doc.fillColor("#1D1D1F").fontSize(13).text("Potential Gaps", 46, alY)
    alY += 25

    const mismatchText = (p4.mismatch_points || []).map((m: string) => "• " + m).join("\n")
    ...
    doc.fillColor("#1D1D1F").fontSize(13).text("Nova's Recommendations", 46, alY)
    alY += 25

    const recommendationsText = (p4.recommendations || []).map((r: string) => "• " + r).join("\n")
    ...
    doc.rect(0, 0, 595, 842).fill("#F5F5F7")
    doc.rect(0, 0, 595, 100).fill("#FFF")
    doc.fillColor("#0A5CFF").fontSize(24).text("Behavioral Overview", 46, 50)
    ...
    metrics.forEach((m) => {
      ...
      doc.fillColor("#1D1D1F").fontSize(9).text(`${m.value}%`, 445, y - 5)
      y += 26
    })
    ...
    doc.fillColor("#1D1D1F").fontSize(11).text(
    ...
    doc.fillColor("#0A5CFF").fontSize(14).text("Global Communication Scores", 46, 580)
    const globalScores = [
    ...
    globalScores.forEach((s) => {
      ...
      doc.fillColor("#1D1D1F").fontSize(9).text(`${val}%`, 445, ys - 5)
      ys += 24
    })
    ...
    doc.rect(0, 0, 595, 842).fill("#F5F5F7")
    doc.rect(0, 0, 595, 100).fill("#FFF")
    doc.fillColor("#0A5CFF").fontSize(24).text("Question-by-Question Feedback", 46, 50)
    ...
    for (const [i, m] of (memoryRows || []).entries()) {
      ...
      const head = `Q${i + 1}${m.score ? ` • Score: ${clamp(m.score)}/100` : ""}`
      ...
      yy = block(doc, "Question", questionText, yy)
      ...
      if (m.clarity_followup_used) {
        ...
        yy = block(doc, "Nova Follow-Up", m.clarity_followup_used, yy)
      }
      ...
      if (firstAns) {
        ...
        yy = block(doc, "Answer", firstAns, yy)
      }
      ...
      if (secondAns) {
        ...
        yy = block(doc, "After Clarification", secondAns, yy)
      }
      ...
      if (m.scoring_axes) {
        ...
        doc.fillColor("#1D1D1F").fontSize(11).text("Axes (per question)", 46, yy)
        yy += 18
      ...
      yy = block(doc, "Nova Feedback", m.feedback || "—", yy)
      ...
      yy = block(doc, "Expected Ideal Answer", expected, yy)
      ...
      if (m.improvement_score) {
        ...
        doc.fillColor("#1D1D1F").fontSize(11).text("Improvement Score", 46, yy)
        yy += 18
        doc.rect(60, yy, 480, 8).fill("#E5E7EB")
        doc.rect(60, yy, (imp / 100) * 480, 8).fill("#0A5CFF")
        doc.fillColor("#1D1D1F").fontSize(9).text(`${imp}%`, 555, yy - 3)
        yy += 28
      }
    ...
    if (hasCases) {
      ...
      doc.rect(0, 0, 595, 842).fill("#F5F5F7")
      doc.rect(0, 0, 595, 100).fill("#FFF")
      doc.fillColor("#0A5CFF").fontSize(24).text("Case Study Performance", 46, 50)
      ...
      Object.keys(groupedByCase).forEach((cid) => {
        ...
        items.forEach(({ q, m }) => {
          ...
          const ans = m?.answer_first || m?.reponse || "—"
          ...
          cy = block(doc, "Question", qText, cy)

          if (cy > 700) {
            doc.addPage()
            cy = 60
          }
          cy = block(doc, "Your Answer", ans, cy)

          if (cy > 700) {
            doc.addPage()
            cy = 60
          }
          cy = block(doc, "Expected Approach", exp, cy)
    ...
    doc.rect(0, 0, 595, 842).fill("#F5F5F7")
    doc.rect(0, 0, 595, 100).fill("#FFF")
    doc.fillColor("#0A5CFF").fontSize(24).text("Final Interview Feedback", 46, 50)
    ...
    fy = block(doc, "Summary", session.final_feedback_summary || "—", fy, 500)

    if (fy > 700) {
      doc.addPage()
      fy = 60
    }
    fy = block(doc, "Detailed Feedback", session.final_feedback_text || "—", fy, 500)
    ...
    if (Array.isArray(session.final_feedback_axes) && session.final_feedback_axes.length > 0) {
      ...
      doc.fillColor("#1D1D1F").fontSize(14).text("Key Improvement Axes", 46, fy)
      fy += 25
      session.final_feedback_axes.forEach((ax: string) => {
        ...
        fy = block(doc, null, "• " + ax, fy, 480, 16)
      })
    }
    ...
    if (session.transcript_full) {
      ...
      doc.rect(0, 0, 595, 842).fill("#F5F5F7")
      doc.rect(0, 0, 595, 100).fill("#FFF")
      doc.fillColor("#0A5CFF").fontSize(24).text("Full Transcript (Cleaned)", 46, 50)
    ...
    /* ---------- FOOTER ---------- */
    doc.fillColor("#86868B").fontSize(9).text("Generated by Nova — AI Interview Coach", 0, 800, {
      align: "center",
    })
    ...
    console.error("❌ Nova PDF V7 Error:", err)
